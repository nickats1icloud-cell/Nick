  ...
  return loop(() => {
    ...
    for (let i = 0; i < dots.length; i += 1) {
      for (let j = i + 1; j < dots.length; j += 1) {
        ...
        ctx.strokeStyle = tone("--ice", 0.1 * (1 - dist / link));
        ctx.lineWidth = 1;
        ctx.stroke();
      }
    }
...
export function speedBars(canvas, { bars = 14, streaks = 20 } = {}) {
  ...
  return loop(() => {
    ...
    glow.addColorStop(0, tone("--ember", 0.16));
    glow.addColorStop(0.55, tone("--ember-deep", 0.06));
    ...
    lines.forEach((line) => {
      ...
      grad.addColorStop(0, tone("--ember-glow", line.alpha));
    ...
    columns.forEach((bar) => {
      ...
      ctx.shadowColor = tone("--ember", 1);
      ctx.shadowBlur = 16;

      const grad = ctx.createLinearGradient(bar.x, top, bar.x, h);
      grad.addColorStop(0, tone("--ember-glow", 0));
      grad.addColorStop(0.25, tone("--ember-glow", bar.alpha * 0.6));
      grad.addColorStop(0.85, tone("--ember", bar.alpha));
      grad.addColorStop(1, tone("--ice", bar.alpha));
      ...
      ctx.fillStyle = tone("--ember-glow", bar.alpha * pulse);
      ctx.fill();
      ctx.shadowBlur = 0;
    });
