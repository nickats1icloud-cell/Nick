      ...
      .map((event) => {
        ...
        return `<article class="hub-card ${open ? "hub-card--rail" : ""}" data-event="${esc(event.id)}">
          ...
          <div class="u-row" style="margin-top:14px">
            ${entry?.points_earned != null ? `<span class="hub-badge hub-badge--ember">${entry.points_earned} πόντοι</span>` : ""}
        ...
        ${ranked
          .map(([userId, points], index) => {
            ...
            return `<tr>
              ...
              <td class="hub-table__num" style="color:hsl(var(--ember));font-weight:600">${points}</td>
            </tr>`;
          })
          .join("")}
      </tbody>
