...
async function loadStats() {
  ...
  const bars = [
    ["Μέλη", members, ""],
    ["Threads", threads, ""],
    ["Απαντήσεις", posts, "hero__bar--accent"],
    ["Πρωταθλήματα", champs, "hero__bar--accent"],
  ];
  render(
    "#hero-telemetry",
    bars
      ...
      .map((row) => {
        ...
        return `<a class="hub-card hub-card--hover u-cut" href="championships.html">
          <div class="u-between" style="margin-bottom:10px">
            <span class="hub-badge ${row.status === "active" ? "hub-badge--ok" : row.status === "completed" ? "" : "hub-badge--accent"}">${esc(row.status === "active" ? "Ενεργό" : row.status === "completed" ? "Ολοκληρωμένο" : "Ερχόμενο")}</span>
      ...
      .map((thread, index) => {
        ...
        return `<a class="hub-row-item" href="forum-thread.html?id=${encodeURIComponent(thread.id)}">
          ...
          <div class="hub-row-item__body">
            <p class="hub-row-item__title u-truncate">${esc(thread.title)}${thread.pinned ? ' <span class="hub-badge hub-badge--brand">Pinned</span>' : ""}</p>
