      ...
      .map((product) => {
        ...
        return `<article class="shop-card" data-product="${esc(product.id)}">
          <div class="shop-card__media">
            ...
            ${product.badge ? `<span class="hub-badge hub-badge--brand" style="position:absolute;top:10px;left:10px">${esc(product.badge)}</span>` : ""}
...
function cartBody() {
  ...
  return `${state.cart
    .map(
      (line, index) => `<div class="cart-line">
        ...
        <div style="min-width:0;flex:1">
          ...
          <p class="u-tiny" style="color:hsl(var(--brand))">${fmtPrice(line.price)}</p>
        </div>
        <span class="qty">
    ...
    <div class="u-between" style="padding-top:14px">
      ...
      <strong style="color:hsl(var(--brand));font-size:1.15rem">${fmtPrice(cartTotal())}</strong>
    </div>`;
}
...
function openCheckout() {
  ...
  const { root, close } = openModal({
    ...
    body: `<div class="u-grid u-grid--2" style="gap:14px">
      ...
      <div class="u-between"><strong>Σύνολο</strong><strong style="color:hsl(var(--brand))">${fmtPrice(cartTotal())}</strong></div>`,
      ...
      ? `<div class="u-stack" style="--gap:12px">${rows
          .map(
            (order) => `<div class="hub-card">
              ...
              <p class="u-between" style="margin-top:10px"><span class="u-small">Σύνολο</span><strong style="color:hsl(var(--brand))">${fmtPrice(order.total)}</strong></p>
            </div>`,
          )
          .join("")}</div>`
