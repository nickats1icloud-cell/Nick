    ...
    .map(
      (item) =>
        `<a class="hub-social" href="${safeUrl(item.href) || "#"}" target="_blank" rel="noopener noreferrer"
            title="${esc(item.label)}" aria-label="${esc(item.label)}" style="--social-color:${BRAND_COLORS[item.key]}">${ICONS[item.key]}</a>`,
    )
    .join("");
}
...
function renderAuthArea() {
  ...
  slot.insertAdjacentHTML(
    ...
    `<a class="hub-btn hub-btn--ghost hub-btn--sm" href="profile.html" style="max-width:150px"><span class="u-truncate">${label}</span></a>
     ${session.isAdmin ? '<a class="hub-btn hub-btn--sm" href="admin.html" style="border-color:hsl(var(--brand)/0.5);color:hsl(var(--brand))">Admin</a>' : ""}
